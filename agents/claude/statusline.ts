...
// Worktrunk worktree data from `wt list --format json`
...
const colors = {
	...
	segment: (bg: string, content: string) =>
		`${BG_COLORS[bg]}${FG_CRUST}${content}${RESET}`,
...
// Get all worktrees from worktrunk
async function getWorktreeData(
	cwd: string,
	env: Record<string, string>,
): Promise<WtWorktree[] | null> {
	...
	try {
		return JSON.parse(output) as WtWorktree[];
	} catch {
		return null;
	}
}

// Get worktrunk session markers (🤖 working, 💬 waiting) for all branches
async function getWorktreeMarkers(
	cwd: string,
	env: Record<string, string>,
	...
	if (output === "failed" || !output) return markers;

	// Each line: worktrunk.state.<branch>.marker {"marker":"🤖","set_at":...}
	for (const line of output.split("\n")) {
		const match = line.match(/^worktrunk\.state\.(.+)\.marker\s+(.+)$/);
		if (!match) continue;
		try {
			const data = JSON.parse(match[2]);
			if (data.marker) markers.set(match[1], data.marker);
		} catch {
			// Skip malformed marker entries
		}
	}
	return markers;
}

// Get custom identity emojis for worktrees (set via git config wt-icon.<branch>)
async function getWorktreeIcons(
	cwd: string,
	env: Record<string, string>,
): Promise<Map<string, string>> {
	...
	const output = await execCommand(
		"git",
		["config", "--get-regexp", "wt-icon\\."],
		cwd,
		env,
	);
	if (output === "failed" || !output) return icons;

	// Each line: wt-icon.<branch> <emoji>
	for (const line of output.split("\n")) {
		const match = line.match(/^wt-icon\.(.+?)\s+(.+)$/);
		if (match) icons.set(match[1], match[2]);
	}
	return icons;
}
...
async function getGitStatus(
	...
	return {
		...
		untracked: lines.filter((l) => GIT_STATUS_PATTERNS.untracked.test(l))
			.length,
	};
}
...
function parseVersion(output: string): string {
	if (!output) return "";
	const match =
		output.match(VERSION_PATTERNS.simple) ||
		output.match(VERSION_PATTERNS.named);
...
async function detectLanguages(
	...
	const detectionPromises = Object.entries(LANGUAGE_CONFIGS).map(
		async ([_langName, config]) => {
	...
	const presentLanguages = detectionResults.filter(
...
async function buildStatusline(
	...
	const projectDir =
		context.workspace?.project_dir || "ERROR: Missing Project Dir";
	...
	// Fire all data sources in parallel — wt calls are additive (graceful fallback if absent)
	const [gitRepo, wtData, wtMarkers, wtIcons] = await Promise.all([
		isGitRepo(projectDir, env),
		withTimeout(getWorktreeData(projectDir, env), 400, null),
		withTimeout(
			getWorktreeMarkers(projectDir, env),
			400,
			new Map<string, string>(),
		),
		withTimeout(
			getWorktreeIcons(projectDir, env),
			400,
			new Map<string, string>(),
		),
	]);
	...
	if (gitRepo) {
		...
		let gitSegment = `  ${branch}`; // ← keep the existing git branch icon
		...
		// Augment with remote ahead/behind from worktrunk data
		const currentWt = wtData?.find((wt) => wt.is_current);
		if (currentWt) {
			if (currentWt.remote.ahead > 0)
				gitSegment += ` ↑${currentWt.remote.ahead}`;
			if (currentWt.remote.behind > 0)
				gitSegment += ` ↓${currentWt.remote.behind}`;
		}

		gitSegment += " ";
		segments.push(colors.segment("yellow", gitSegment));

		// Worktree roster — shows other worktrees with identity + session status
		// Hidden when there's only one worktree (just main)
		if (wtData && wtData.length > 1) {
			const others = wtData.filter((wt) => !wt.is_current);
			const entries = others.map((wt) => {
				// Identity: custom emoji (git config wt-icon.<branch>) or truncated branch name
				const icon = wtIcons.get(wt.branch);
				const identity = icon || wt.branch.slice(-5);
				// Status: session marker appended when a Claude session is active
				const marker = wtMarkers.get(wt.branch) || "";
				return `${identity}${marker}`;
			});
			segments.push(colors.segment("pink", ` ${entries.join(" ")} `));
		}
	}
