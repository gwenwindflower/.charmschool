...
wk.add({
  { "<LocalLeader>ua", mode = { "n", "x" }, group = "align", icon = { icon = " ", color = "azure" } },
})
wk.add({
  { "<LocalLeader>u", mode = { "n", "x" }, group = "utils", icon = { icon = " ", color = "yellow" } },
})
wk.add({
  { "<LocalLeader>us", mode = { "n", "x" }, group = "snippet", icon = { icon = " ", color = "green" } },
})
wk.add({
  { "<LocalLeader>uj", "<cmd>%!jq -S<cr>", mode = { "n" }, desc = "Sort JSON keys with jq" },
})
