# Claude Code Agent Guidelines (Charm School)

> [!NOTE]
> Use progressive disclosure to keep this document concise. Take advantage of rules, directory-scoped context files, and imports as described in the [Claude Code docs on project memory](https://code.claude.com/docs/en/memory#modular-rules-with-claude-rules)
...
- `agents/` — coding agent configs. Cross-agent resources live in `agents/shared/` (skills, rules, prompts, agent definitions), agent-specific configs in `agents/<agent>/` (e.g. `agents/claude/`).
  - `agents/shared/` is a rotz dot that symlinks to both `~/.agents/` and `~/.claude/` — one source of truth, two discovery paths.
  - `agents/<agent>/` directories hold agent-specific configs (settings, keybindings, etc.) and can depend on `shared` to inherit cross-agent resources.
